...
public class Des {
     ...
     * @return
     */
    public byte[] DataFormat(byte[] data) {
     ...
     * @param result_data
     */
    public byte[] EcbModel(int flag, byte[] format_key, byte[] format_data, int datalen, byte[] result_data) {
     ...
     * @return byte数组
     */
    public byte[] UnitDes(byte[] des_key, byte[] des_data, int flag) {
     ...
     * @return
     */
    public int[] ReadDataToBirnaryIntArray(byte[] intdata) {
        int i;
        int j;
        int[] IntDa = new int[8];
    ...
    int[] K0 = new int[56];// 初始密钥记作Y0，它由64位01序列组成，但其中8位用于奇偶校验，所以实际用于加密的只有56位，记为K0
     ...
     */
    public int[][] KeyInitialize(int[] key, int[][] keyarray) {
        int i;
        int j;
        for (i = 0; i < 56; i++) {
    ...
    public byte[] Encrypt(int[] encryptdata, int flag, int[][] keyarray) {
        int i;
        // 通过初始置换函数对明文进行初始置换
        for (i = 0; i < 64; i++) {
     ...
     */
    public int[] LoopF(int[] M, int times, int flag, int[][] keyarray) {
        int i;
        int j;
        for (i = 0; i < 32; i++) {
            L0[i] = M[i]; // 明文左侧的初始化
     ...
     * @return
     */
    public byte[] BirnaryIntArrayToInt(int[] data) {
        int i;
        int j;
     ...
     * 根据byte数组，生成文件
     */
    public void byteArrayToFile(byte[] bfile, String filePath, String fileName) {
     ...
     */
    public String EncryptFile(String srcFilePath,String descPath,String descFileName, int flag) {
        File file = new File(srcFilePath);
        byte[] fileByte = getFileByte(file.getAbsolutePath());
        if (flag == 1) {
            // 加密
            byte[] result = DesEncrypt(fileByte, 1);
            //byteArrayToFile(result, "src\\files", file.getName() + "的密文");
            byteArrayToFile(result, descPath,descFileName);
            //return "src\\files\\" + file.getName() + "的密文";
        } else if (flag == 0) {
            byte[] result = DesEncrypt(fileByte, 0);
            //byteArrayToFile(result, "src\\receive", file.getName().replace("的密文", ""));
            byteArrayToFile(result, descPath,descFileName);
            return "src\\receive\\" + file.getName().replace("的密文", "");
        }
        return "";
    }
}
